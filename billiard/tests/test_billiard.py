#
# Unit tests for the billiard package
#
...
test_support.import_module('billiard.synchronize')
...
try:
    from billiard.sharedctypes import Value, copy
...
class _TestValue(BaseTestCase):
    ...
    def setUp(self):
        if not HAS_SHAREDCTYPES:
            self.skipTest("requires billiard.sharedctypes")
...
class _TestPool(BaseTestCase):
    ...
    def test_make_pool(self):
        self.assertRaises(ValueError, billiard.Pool, -1)
        self.assertRaises(ValueError, billiard.Pool, 0)
...
class _TestPoolWorkerLifetime(BaseTestCase):
    ...
    def test_pool_worker_lifetime_early_close(self):
        ...
        p = billiard.Pool(3, maxtasksperchild=1)
        results = []
        for i in range(6):
...
class _TestConnection(BaseTestCase):
    ...
    @unittest.skipUnless(HAS_REDUCTION, "test needs billiard.reduction")
...
class _TestHeap(BaseTestCase):
    ...
    def test_free_from_gc(self):
        ...
        for i in range(5000):
            a = billiard.heap.BufferWrapper(1)
            b = billiard.heap.BufferWrapper(1)
            # circular references
            a.buddy = b
            b.buddy = a
...
class _TestFinalize(BaseTestCase):
    ...
    def _test_finalize(cls, conn):
        ...
        # call billiard's cleanup function then exit process without
...
class _TestImportStar(BaseTestCase):
    ...
    def test_import(self):
        modules = [
            ...
            'billiard.pool', 'Process',
            'billiard.synchronize', 'billiard.util'
        ]

        if HAS_REDUCTION:
            modules.append('billiard.reduction')
...
def _test_process(q):
    ...
    subProc = Process(target=_this_sub_process, args=(queue,))
    subProc.daemon = True
    subProc.start()
    subProc.join()
...
class TestStdinBadfiledescriptor(unittest.TestCase):
    def test_queue_in_process(self):
        queue = billiard.Queue()
        proc = Process(target=_test_process, args=(queue,))
        proc.start()
        proc.join()

    def test_pool_in_process(self):
        p = Process(target=pool_in_process)
        p.start()
        p.join()
    ...
    def test_flushing(self):
        ...
        flike.write('foo')
        proc = Process(target=lambda: flike.flush())
        flike.flush()
...
class TestTimeouts(unittest.TestCase):
    ...
    def _test_timeout(cls, child, address):
        ...
        child.send(123)
        child.close()
        conn = billiard.connection.Client(address)
        conn.send(456)
        conn.close()
    ...
    def test_timeout(self):
        ...
        try:
            ...
            parent, child = billiard.Pipe(duplex=True)
            l = billiard.connection.Listener(family='AF_INET')
            p = Process(target=self._test_timeout,
...
class TestFlags(unittest.TestCase):
    ...
    def run_in_child(cls):
        ...
        r, w = billiard.Pipe(duplex=False)
        p = Process(target=cls.run_in_grandchild, args=(w,))
        p.start()
...
class TestForkAwareThreadLock(unittest.TestCase):
    ...
    def child(cls, n, conn):
        if n > 1:
            p = Process(target=cls.child, args=(n - 1, conn))
            p.start()
            p.join()
        else:
    ...
    def test_lock(self):
        r, w = billiard.Pipe(False)
        l = util.ForkAwareThreadLock()
        old_size = len(util._afterfork_registry)
        p = Process(target=self.child, args=(5, w))
        p.start()
        new_size = r.recv()
        p.join()
...
class TestIgnoreEINTR(unittest.TestCase):
    ...
    def test_ignore(self):
        conn, child_conn = billiard.Pipe()
        try:
            p = Process(target=self._test_ignore,
                        args=(child_conn,))
            p.daemon = True
            p.start()
            child_conn.close()
    ...
    def _test_ignore_listener(cls, conn):
        ...
        signal.signal(signal.SIGUSR1, handler)
        l = billiard.connection.Listener()
        conn.send(l.address)
        a = l.accept()
        a.send('welcome')
    ...
    def test_ignore_listener(self):
        conn, child_conn = billiard.Pipe()
        try:
            p = Process(target=self._test_ignore_listener,
                        args=(child_conn,))
            p.daemon = True
            p.start()
            child_conn.close()
            ...
            os.kill(p.pid, signal.SIGUSR1)
            time.sleep(0.1)
            client = billiard.connection.Client(address)
